...
func PrometheusMetrics(namespace string, labelsAndValues ...string) *Metrics {
	...
	return &Metrics{
		...
		VerificationBudgetDrops: prometheus.NewCounterFrom(stdprometheus.CounterOpts{
			Namespace: namespace,
			Subsystem: MetricsSubsystem,
			Name:      "verification_budget_drops",
			Help:      "Number of peer Vote and Commit messages dropped because the signature-verification budget was exhausted.",
		}, labels).With(labelsAndValues...),
		PeerLaneDrops: prometheus.NewCounterFrom(stdprometheus.CounterOpts{
			Namespace: namespace,
			Subsystem: MetricsSubsystem,
			Name:      "peer_lane_drops",
			Help:      "Number of queued peer consensus messages dropped by the per-peer scheduler.",
		}, labels).With(labelsAndValues...),
		BlockPartProofDrops: prometheus.NewCounterFrom(stdprometheus.CounterOpts{
			Namespace: namespace,
			Subsystem: MetricsSubsystem,
			Name:      "block_part_proof_drops",
			Help:      "Number of peer block parts dropped before proof verification because the sender's invalid-proof allowance was spent.",
		}, labels).With(labelsAndValues...),
		StateChannelDrops: prometheus.NewCounterFrom(stdprometheus.CounterOpts{
			Namespace: namespace,
			Subsystem: MetricsSubsystem,
		...
		ProposalVerifyFailures: prometheus.NewCounterFrom(stdprometheus.CounterOpts{
			Namespace: namespace,
			Subsystem: MetricsSubsystem,
			Name:      "proposal_verify_failures",
			Help:      "Number of peer proposals whose signature failed verification.",
		}, labels).With(labelsAndValues...),
		...
		PeerVoteVerifyLatencySeconds: prometheus.NewHistogramFrom(stdprometheus.HistogramOpts{
			Namespace: namespace,
			Subsystem: MetricsSubsystem,
			Name:      "peer_vote_verify_latency_seconds",
			Help:      "Seconds an accepted peer vote spent from being queued by the reactor to being added to the vote set.",

			Buckets: []float64{0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 0.25, 0.5, 1, 2, 5},
		}, labels).With(labelsAndValues...),
		VerificationBudgetSaturation: prometheus.NewGaugeFrom(stdprometheus.GaugeOpts{
			Namespace: namespace,
			Subsystem: MetricsSubsystem,
			Name:      "verification_budget_saturation",
			Help:      "Fraction of the node-wide verification budget still available when a peer message is checked, from 1.0 full and idle to 0.0 drained and under pressure.",
		}, labels).With(labelsAndValues...),
		PeerLaneActiveCount: prometheus.NewGaugeFrom(stdprometheus.GaugeOpts{
			Namespace: namespace,
			Subsystem: MetricsSubsystem,
			Name:      "peer_lane_active_count",
			Help:      "Number of per-peer lanes currently holding queued messages and taking turns in the scheduler rotation.",
		}, labels).With(labelsAndValues...),
		PeerLaneMaxDepth: prometheus.NewGaugeFrom(stdprometheus.GaugeOpts{
			Namespace: namespace,
			Subsystem: MetricsSubsystem,
...
func NopMetrics() *Metrics {
	return &Metrics{
		...
		VerificationBudgetDrops:      discard.NewCounter(),
		...
		PeerVoteVerifyLatencySeconds: discard.NewHistogram(),
