...
import (
	...
	testfactory "github.com/dashpay/tenderdash/internal/test/factory"
		...
		*/
		for _, tc := range testCases {
			t.Run(fmt.Sprintf("H:%d/%s", height, tc.name), func(t *testing.T) {
				...
				err = blockExec.ValidateBlockWithRoundState(ctx, state, changes, block, types.CommitVerification{})
...
func TestValidateBlockCommit(t *testing.T) {
	...
	for height := int64(1); height < validationTestsStopHeight; height++ {
		...
		*/
		...
		badVote.VoteExtensions = types.MustVoteExtensionsFromProto(t, b.VoteExtensions...)
