...
func (c *Local) GenesisChunked(ctx context.Context, id uint) (*coretypes.ResultGenesisChunk, error) {
	...
	return c.env.GenesisChunked(ctx, &coretypes.RequestGenesisChunked{Chunk: coretypes.Int64(chunkID)})
}
...
func (c *Local) resubscribe(ctx context.Context, subArgs pubsub.SubscribeArgs) eventbus.Subscription {
	...
	for {
		...
		attempts++
		timer.Reset((10 << tmmath.MustConvertUint(attempts)) * time.Millisecond) // 10ms -> 20ms -> 40ms
		select {
		case <-timer.C:
			continue
		case <-ctx.Done():
