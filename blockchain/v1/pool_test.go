...
func makeBlockPool(bcr *testBcR, height int64, peers []BpPeer, blocks map[int64]tPBlocks) *BlockPool {
	...
	for h, p := range blocks {
		...
		if p.create {
			...
			_ = bPool.peers[p.id].AddBlock(types.MakeBlock(h, coreChainLock.BlockHeight, &coreChainLock, txs, nil, nil), 100)
		}
	}
	return bPool
}
...
func TestBlockPoolAddBlock(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "block from unknown peer",
			...
			args: args{
				peerID:    "P2",
				block:     types.MakeBlock(int64(10), coreChainLock.BlockHeight, &coreChainLock, txs, nil, nil),
				blockSize: 100,
			},
		...
		{name: "unexpected block 11 from known peer - waiting for 10",
			...
			args: args{
				peerID:    "P1",
				block:     types.MakeBlock(int64(11), coreChainLock.BlockHeight, &coreChainLock, txs, nil, nil),
				blockSize: 100,
			},
