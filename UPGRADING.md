...
This release bounds the signature-verification work an unauthenticated peer can
force on a node and shares the remaining budget fairly between peers. Most of it
needs no operator action. Two config values, however, change behaviour in a way
that can stop a node from starting or quietly cost it votes. **Both affect only
hand-written `config.toml` files** — nodes whose config is generated by dashmate
receive the new defaults and are unaffected.

### `verification-rate-limit` has a minimum and will now reject a too-low value

The node-wide budget for peer signature verification, in verification operations
per second (`[consensus] verification-rate-limit`, default `300`), must now be
either `0` (disabled) or at least `1 + MaxVoteExtensions` — currently **33**. A
config that sets it to any value between `0` and `33` will fail validation and
**the node will not start**. Raise it to at least `33`, or remove the line to
take the default.

### `peer-vote-rate-limit` changed default *and* units

The per-peer limit on the vote channel (`[consensus] peer-vote-rate-limit`)
previously counted **messages per second** and defaulted to `100`. It now counts
**verification work per second** and defaults to `600`. In the new units a
prevote costs 1, a typical four-extension precommit costs 10, and a
maximum-extension precommit costs 33.

A config file that still carries the old `peer-vote-rate-limit = 100` keeps the
number under the new meaning, which allows only about ten realistic precommits
per second from each peer and will silently drop the rest during vote gossip and
catch-up. **Remove the line to take the new default, or set it in the new units**
(≈ 600 for the previous headroom). A node exceeding the limit is never punished —
its messages are dropped locally — so the symptom is missed votes and slower
rounds, not disconnection.

### Metrics

One metric is removed and several are added; see
[docs/nodes/metrics.md](docs/nodes/metrics.md). The removed
`consensus_vote_gate_fail_open` gauge accompanied a vote-relay filter that this
release deletes. If you built alerts or dashboards on it, drop them; nothing in
the shipped tooling references it.

## State sync: `statesync.fetchers` now trades memory for backfill speed

State-sync backfill verifies the threshold signature on every commit it walks back
over, where it previously compared hashes only. That verification runs in a single
loop while `[statesync] fetchers` (default `4`) workers fetch ahead of it, and
nothing throttles the fetchers when verification falls behind: blocks already
fetched but not yet verified are held in memory, bounded only by the length of the
backfill, which is one evidence window (`MaxAgeNumBlocks`).

At the default the two sides run at roughly comparable rates and the held set stays
small. Raising `fetchers`, or syncing from unusually fast or nearby peers, lets
fetching outrun verification, and the held set then grows toward the whole window —
so a wider evidence window costs proportionally more memory. **No action is needed
to take the default.** If you raise `fetchers` to shorten a long backfill, size the
node's memory for the window it will be walking.
...
Core. Sections below this point predate the Tenderdash fork and are retained for
