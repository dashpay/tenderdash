...
func mustGenerateLLMQData(proTxHashes []crypto.ProTxHash, m int, sigHash bls.Hash) *LLMQData {
	llmqData, err := NewLLMQData(
		proTxHashes,
		WithThreshold(m),
...
func llmqTestDataGetter(n, m int, sigHash bls.Hash) func() llmqTestCaseData {
	return func() llmqTestCaseData {
		llmqData := mustGenerateLLMQData(CreateProTxHashes(n), m, sigHash)
...
func proTxHashes2BLSHashes(proTxHashes []crypto.ProTxHash) []bls.Hash {
	...
	for i, proTxHash := range ReverseProTxHashes(proTxHashes) {
